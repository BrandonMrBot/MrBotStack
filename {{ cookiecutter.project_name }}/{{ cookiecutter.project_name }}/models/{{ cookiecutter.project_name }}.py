...
class User(Base):
    ...
    user_cnty = Column(ForeignKey("country.cnty_cod"), nullable=True)
    user_organization = Column(Unicode(120))
    user_active = Column(ForeignKey("status.status_id"), nullable=True)
    user_role = Column(ForeignKey("role.role_id"), nullable=True)
